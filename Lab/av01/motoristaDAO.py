...
class MotoristaDAO:
    ...
    def __init__(self):
        self.db = Database("Database", "Motoristas")
    ...
    def __serialize_motorista(self, motorista: Motorista):
        corrida_data = [
            self.__serialize_corrida(corrida) for corrida in motorista.corrida
        ]
        return {"nota": motorista.nota, "corrida": corrida_data}

    def __deserialize_motorista(self, data: Dict[str, Any]):
        corrida_data = data["corrida"]
        corrida = [
            self.__deserialize_corrida(cdata) for cdata in corrida_data
        ]
        return Motorista(data["nota"], corrida)
    ...
    def __deserialize_corrida(self, data: Dict[str, Any]):
        ...
        return Corrida(data["nota"], data["distancia"], data["valor"],  passageiro)